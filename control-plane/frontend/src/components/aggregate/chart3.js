...
function Chart3(props) {
  ...
  const getData = async () => {
    ...
    const url = `https://${process.env.REACT_APP_DATA_PLANE_DOMAIN}/group-detail/client-fail-ranking?group-name=` + groupName;
  ...
  return (
    <>
      <Box bg="white" w="40vw" h="80vh" borderRadius="20px" boxShadow="lg">
              ...
              {clientData.map((info, index) => (
                <Tr key={index}>
                  ...
                  <Td isNumeric border="none">
                    <div style={{ margin: "10% auto", textAlign: "center", paddingLeft: "50%" }}>
                      {info.count}
                    </div>
                  </Td>
                  <Td isNumeric border="none">
