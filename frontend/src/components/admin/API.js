...
const API = () => {
    ...
    const GetProduct = async(apisName) => {
        try{
            const ProductData = await CustomAxios.get('apis/user',{
                params: {
                    name: apisName,
                }
            });
    ...
    const GetSubscription = async() => {
        try {
            const SubscriptionData = await CustomAxios.get('subscription/user',{
                params: {
    ...
    const ApplySubscription = async() => {
        ...
        try {
            await CustomAxios.post("subscription/user/apply",data);
            console.log(data);
    ...
    useEffect(() => {
        console.log("apis id : ", apiId);
        console.log(JSON.stringify({
            "openapi": "3.0.3",
            "info": {
              "title": "히히 승기쓰 API인디 - CloseAPI 6.0",
              "description": "여기는 어떻게 바뀌는지 궁금해서 내가 직접 수정해본 곳이야. 잘 나오니 ?",
